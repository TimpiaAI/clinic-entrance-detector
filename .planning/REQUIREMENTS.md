...
- [ ] **BACK-02**: POST /api/process/start starts the detector subprocess
- [ ] **BACK-03**: POST /api/process/stop stops the detector subprocess (with psutil tree kill on Windows)
- [ ] **BACK-04**: GET /api/process/status returns detector process state
- [ ] **BACK-05**: POST /api/transcribe accepts audio blob (WebM), transcribes with Faster Whisper, returns text + extracted CNP/email
